...
class DisburseLoanResource:
    ...
    def on_post(self, req, resp):
        ...
        output_dict = {"msgHeader": {"authToken": ""}, "data": {
            "error": 0, "message": "could not disburse loan"}}
        errors = utils.errors
        success = "loan disbursed successfully"
        logInfo = {'api': 'disburseLoan'}
        try:
            raw_json = req.stream.read()
        ...
        try:
            if not validate.Request(api='approveLoan', request=input_dict):
                ...
                if val_error:
                    ...
                    payload = {"transactionAmount": input_dict["principal"] if "principal" in input_dict else "5500",
                               "actualDisbursementDate": datetime.now().strftime("%d %B %Y"), "dateFormat": "dd MMMM yyyy",
                               "paymentTypeId": "2", "locale": "en"}
                    print(payload)
                    print(baseurl)
                    utils.logger.info("api URL: " + baseurl + "loans/" + input_dict["data"]["loanID"] + "?command=disburse", extra=logInfo)
                    utils.logger.info("api Request: " + json.dumps(payload), extra=logInfo)
                    r = requests.post(baseurl + "loans/" + input_dict["data"]["loanID"] + "?command=disburse", data=json.dumps(payload),headers=headers, auth=auth, verify=False)
                    print(r)
                    print(requests.post(baseurl + "loans/" + input_dict["data"]["loanID"] + "?command=disburse", data=json.dumps(payload),headers=headers, auth=auth, verify=False))
                    utils.logger.info("api response: " + json.dumps(r.json()), extra=logInfo)
                    print(r.status_code==200)
                    if r.status_code == 200:
                        updated = db.Update(db="mint_loan", table="mw_client_loan_master", checkAll=False,conditions={"ID = ": str(input_dict["data"]["id"])}, STATUS="ACTIVE")
                        if updated:
                        ...
                        input_dict["data"].update(
                            {"error": 1, "message": "could not disburse loan"})
                resp.body = json.dumps(output_dict)
                utils.logger.debug(

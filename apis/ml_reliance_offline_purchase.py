    ...
    def on_post(self, req, resp, **kwargs):
        ...
        try:
            data = {"docType": req.get_param(
                "docType"), "format": req.get_param("format")}
            ...
            try:
                ...
                if val_error:
                    ...
                    if docID["data"] != []:
                        ...
                        docID = None
                    junk = s3.Object(
                        bucket, folder + filename).put(Body=purchaseData.file.read())
                    junk = s3.Bucket(bucket).download_file(
                        folder + filename, "/tmp/purchase." + suffix)
                    ...
                    payload = {
                        "req": [{header[i]:data[i].value for i in range(len(data))} for data in d[1:]]}
                    #print(payload, header)
                    #url = "http://13.126.29.47:8080/MintReliance/MintReliance/relianceOfflinePurchase"
                    url = "https://dev.mintwalk.com/tomcat/MintReliance/MintReliance/relianceOfflinePurchase"
                    ...
                    if r.status_code == 200:
                        ...
                        if "token" in list(token.keys()):
                            output_dict["data"].update(
                                {"error": 0, "message": success, "response": r.json()})

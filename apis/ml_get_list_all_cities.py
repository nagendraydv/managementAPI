        ...
        try:
            if not validate.Request(api='companyGroup', request=input_dict):
                ...
                if val_error:
                    ...
                    q1=Query.from_(cityMaster).select(cityMaster.CITY_ID,cityMaster.CITY,cityMaster.STATE,cityMaster.DISABLED)
                    city=db.runQuery(q1)["data"]
                    if city!=[]:
                        token = generate(db).AuthToken()
                        if token["updated"]:
                            output_dict["data"].update({"error": 0, "message": "company details found","city":city})
                            output_dict["msgHeader"]["authToken"] = token["token"]
                        else:
                            output_dict["data"].update({"error": 1, "message": errors["token"]})
                    else:
                        token = generate(db).AuthToken()
                        output_dict["data"].update({"error": 0, "message":"company details not found"})
                        output_dict["msgHeader"]["authToken"] = token["token"]    
                resp.body = json.dumps(output_dict)
                db._DbClose_()
        except Exception as ex:

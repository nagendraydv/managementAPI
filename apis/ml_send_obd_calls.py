...
class SendBulkObdCallsUploadResource:
    ...
    def on_post(self, req, resp, **kwargs):
        ...
        try:
            ...
            callData = req.get_param("callData")
        ...
        if ((not validate.Request(api='incomeDataUpload', request={"msgHeader": msgHeader, "data": data})) or
                (callData.filename.split('.')[-1] not in ("xlsx", "xls"))):
            output_dict["data"].update({"error": 1, "message": errors["json"]})
        else:
            try:
                suffix = callData.filename.split('.')[-1]
                filename = callData.filename  # + suffix
            ...
            try:
                ...
                if val_error:
                    ...
                    junk = s3.Object(bucket, folder +
                                     filename).put(Body=callData.file.read())
                    junk = s3.Bucket(bucket).download_file(
                        folder + filename, "/tmp/bulk_call." + suffix)
                    md = xlrd.open_workbook(
                        filename="/tmp/bulk_call." + suffix, encoding_override='unicode-escape')
                    sheet = md.sheet_by_index(0)
                    d = [sheet.row_slice(i) for i in range(sheet.nrows)]
                    mapping = {"call_type": "CALL_TYPE", "language": "LANGUAGE",
                    ...
                    if mobInd and cidInd:
                        ...
                        phoneNumbers = None
                    if phoneNumbers:
                        payload = {"callType": d[1][ind[h.index("CALL_TYPE")]].value, "language": d[1][ind[h.index("LANGUAGE")]].value,
                        ...
                        for i in range(0, len(phoneNumbers), 100):
                            ...
                            r = requests.post("http://13.126.29.47:8080/mintLoan/mintloan/sendObdCall", data=json.dumps(payload),
                                              headers={"Content-Type": "application/json"}, verify=False)  # sendSmsToMultipleNumbers
                        if r.status_code == 200:
                            token = generate(db).AuthToken()
                            if "token" in list(token.keys()):
                                output_dict["data"].update(
                                    {"error": 0, "message": success, "resp": r.json()})
                                output_dict["msgHeader"]["authToken"] = token["token"]
                            else:
                                output_dict["data"].update(
                                    {"error": 1, "message": errors["token"], "resp": r.json()})
                        else:
                            output_dict["data"].update(

...
class RecoveryPaymentsBulkUploadResource:
    ...
    def on_post(self, req, resp, **kwargs):
        ...
        logInfo = {'api': 'recoverypayBulkUpload'}
        try:
        ...
        if ((not validate.Request(api='incomeDataUpload', request={"msgHeader": msgHeader, "data": data})) or
            ...
            try:
                ...
                if val_error:
                    ...
                    loans = []
                    gen = generate(db)
                    gen.DBlog(logFrom="recoverypayBulkUpload", lineNo=inspect.currentframe().f_lineno,
                    ...
                    for i, datum in enumerate(d[1:]):
                        ...
                        postUrl = baseurl + "loans/" + \
                            str(int(x["loan_id"])) + \
                            "/transactions?command=recoverypayment"
                        gen.DBlog(logFrom="recoverypayBulkUpload", lineNo=inspect.currentframe().f_lineno,
                        ...
                        loans.append(str(int(x["loan_id"])))

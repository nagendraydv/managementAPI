...
class GetUnidentifiedUberDataResource:
        ...
        try:
            if False:  # not validate.Request(api='', request=input_dict):
                ...
                if val_error:
                    ...
                    q = Query.from_(income).select(income.star).where((income.CUSTOMER_ID == '0') & (income.IGNORE == '0')).groupby(income.CONTACT_NUMBER)
                    print(q)
                    respdict = db.runQuery(q)
                    #print(respdict)
                    
                    if True:  # token["updated"]:
                        output_dict["data"]["unidentifiedData"] = utils.camelCase(respdict["data"])
